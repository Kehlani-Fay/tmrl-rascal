...
class DummyRCDroneInterface(RealTimeGymInterface):
    ...
    def get_observation_space(self):
        ...
        human_x_space = spaces.Box(low=-1.0, high=1.0, shape=(1,)) 
        human_y_space = spaces.Box(low=-1.0, high=1.0, shape=(1,))
        zeta_pred_x = spaces.Box(low=-1.0, high=1.0, shape=(1,)) 
        zeta_pred_y = spaces.Box(low=-1.0, high=1.0, shape=(1,)) 
        #self.action_space = spaces.Box( np.array([-1,0,0]), np.array([+1,+1,+1]))  # steer, gas, brake
        return spaces.Tuple((pos_x_space, pos_y_space, tar_x_space, tar_y_space, zeta_pred_x, zeta_pred_y, ))
...
class MyMemory(TorchMemory):
    ...
    def append_buffer(self, buffer):
        ...
        if self.__len__() > 0:
            ...
            self.data[9] += list_x_position
            self.data[10] += list_x_position
        else:
