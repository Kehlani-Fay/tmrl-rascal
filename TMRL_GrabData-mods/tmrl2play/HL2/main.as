...
void Main()
{
	...
	while(true)
	{
		...
		MemoryBuffer@ humanBuf = MemoryBuffer(0);

		//csv init
		string human1Name = "humansData.csv";
		string human2Name = "humansData.csv";
		file player1CSV;
		file player2CSV;
		...
		while(cc)
		{
			...
			if(arena.Players.Length >= 2)
			{
				...
				append_float(humanBuf, humanApi.Speed);
				append_float(humanBuf, humanApi.Distance);
				append_float(humanBuf, humanApi.Position.x);
				append_float(humanBuf, humanApi.Position.y);
				append_float(humanBuf, humanApi.Position.z);
				append_float(humanBuf, humanApi.InputSteer);
				append_float(humanBuf, humanApi.InputGasPedal);

				if(humanApi.InputIsBraking) append_float(humanBuf, 1.0f);
				else append_float(humanBuf, 0.0f);

				if(human_race_state == SGamePlaygroundUIConfig::EUISequence::Finish) append_float(humanBuf, 1.0f);
				else append_float(humanBuf, 0.0f);
				
				append_float(humanBuf, humanApi.EngineCurGear);
				append_float(humanBuf, humanApi.EngineRpm);

				//write data to csv
				if(player1CSV.open(human1Name, "w") >= 0 ) 
				{
					// Read the whole file into the string buffer 
					player1CSV.writeFloat(humanApi.Speed);
					player1CSV.writeFloat(humanApi.Distance);
					player1CSV.writeFloat(humanApi.Position.x);
					player1CSV.writeFloat(humanApi.Position.y);
					player1CSV.writeFloat(humanApi.Position.z);
					player1CSV.writeFloat(humanApi.InputSteer);
					player1CSV.writeFloat(humanApi.EngineCurGear);
					player1CSV.writeFloat(humanApi.EngineRpm);
				}
