...
void Main()
{
	while(true)
	{
		...
		while(cc)
		{
			...
			{
				MemoryBuffer@ humanBuf = MemoryBuffer(1);

				if (!sock.Write(humanBuf))
				{
					print("INFO: Human Socket Disconnected, could not send data.");
					return false;
				}

				auto humanPlayer = arena.Players[1];
				CSmScriptPlayer@ humanApi = cast<CSmScriptPlayer>(humanPlayer.ScriptAPI);
				auto race_state = playground.GameTerminals[1].UISequence_Current;
				humanBuf.Seek(0, 0);
				...
				if(race_state == SGamePlaygroundUIConfig::EUISequence::Finish) append_float(humanBuf, 1.0f);
				else append_float(humanBuf, 0.0f);
				

				append_float(humanBuf, api.EngineCurGear);
				append_float(humanBuf, api.EngineRpm);
				humanBuf.Seek(0, 0);
				
				cc = send_memory_buffer(sock, humanBuf);
